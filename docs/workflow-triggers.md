...
Workflow triggers (`IWorkflowTrigger`) define the events or conditions that initiate your Atom workflows. They allow you
to specify exactly when your CI/CD pipelines should run, whether it's on code pushes, pull requests, a schedule, or
manual intervention.

## `IWorkflowTrigger` Interface

This is a marker interface for all workflow trigger types. You won't implement it directly, but rather use its concrete
implementations.
...
**What it is:**
A trigger that responds to `git push` events. It allows for fine-grained control over which pushes activate the workflow
based on branches, file paths, and tags.
...
**What it is:**
A trigger that responds to PR-related events (e.g., PR opened, synchronized, reopened). It allows filtering based on
target branches, file paths, and PR event types.
...
**What it is:**
A trigger that enables users to manually start a workflow, typically from the CI/CD platform's UI. It can be configured
to prompt the user for specific input values.
...
* **`Inputs`**: A list of `ManualInput` objects (`ManualStringInput`, `ManualBoolInput`, `ManualChoiceInput`) that
  define the parameters the user will be prompted for.
...
**What it is:**
A trigger that automatically runs a workflow at specified times or intervals, using a CRON expression.
...
By combining these workflow triggers, you can precisely control the automation of your build and deployment pipelines,
ensuring they run exactly when and how you need them to.
