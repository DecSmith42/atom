...
///     instead of relying on the default artifact handling mechanisms of the CI/CD platform (e.g., GitHub Actions, Azure
///     DevOps).
/// </summary>
/// <remarks>
///     <para>
///         When <see cref="ToggleWorkflowOption{TSelf}.Enabled" />, this signals the workflow generation process to
///         utilize
///         the <see cref="IStoreArtifact.StoreArtifact" /> and <see cref="IRetrieveArtifact.RetrieveArtifact" /> targets.
///         These targets then delegate artifact operations to the registered <see cref="IArtifactProvider" />
///         implementation.
///     </para>
///     <para>
///         When this option is not enabled (or explicitly <see cref="ToggleWorkflowOption{TSelf}.Disabled" />),
///         the Atom framework will typically use the native artifact storage capabilities of the underlying CI/CD
///         platform.
///         For example, in GitHub Actions, it would use <c>actions/upload-artifact</c> and
///         <c>actions/download-artifact</c>.
///         In Azure DevOps, it would use <c>PublishPipelineArtifact@1</c> and <c>DownloadPipelineArtifact@2</c>.
///     </para>
///     <para>
///         Enabling this option is necessary if you intend to use a specific artifact storage backend, such as Azure Blob
///         Storage,
///         across different CI/CD platforms or for local builds, providing a consistent artifact management strategy.
///         The <see cref="WorkflowResolver" /> checks <c>UseCustomArtifactProvider.IsEnabled(workflowOptions)</c> to
...
/// ];
/// </code>
///     In this example, the "Test_BuildWithCustomArtifacts" workflow has `UseCustomArtifactProvider.Enabled` in its
///     options, meaning it will
